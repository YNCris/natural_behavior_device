function plot_skl_dl(X,Y,Z,plotstr,linestr,drawline)
%{
    绘制一帧对齐的骨架点
%}
%% 画点
...
% Z = XYZ(3:3:end);
for k = 1:size(X,1)
    plot3(X(k,1),Y(k,1),Z(k,1),'.','Color',plotstr,'MarkerSize',3)
    hold on
end
hold off
%% 划线
...
for l = 1:size(drawline,1)
    ...
    line(pts(:,1), pts(:,2), pts(:,3),'color',linestr,'linewidth',0.25)
end
